...
- **额度**页签在每次打开或刷新时自动读取周额度。今天展示分钟快照，多天范围展示每日最后一次采集值，分别保留额度类别和重置时间。
...
- `~/.codex-usage-edge/` 中的 `history.json` 保存用量历史，`report.json` 缓存合并报告，`quota.json` 保存分钟额度快照。同一分钟、同类别、同重置周期保留最新值；原手工记录继续保留。
- 额度通过本机已登录 Codex 的 `app-server` 接口 `account/rateLimits/read` 获取，与 ccusage 独立。仅读状态，不创建 AI 任务、不兑换额度重置信用。需要兼容版本的 Codex，可用 `CODEX_USAGE_CODEX_BIN` 指定程序路径。失败保留旧快照并显示错误。仅在打开或刷新时采集，不在后台每分钟轮询；百分比精度以服务端返回值为准。
...
自动化测试覆盖数据规范化、历史迁移、分钟去重、周窗口识别、失败保留、版本回退和 Native Messaging。使用上述命令验证，更新后在 Edge 中重新加载扩展。
...
| 额度图表为空 | 确认本机已安装并登录兼容版本的 Codex，点击刷新。首次采集之前的额度历史无法回溯。 |
