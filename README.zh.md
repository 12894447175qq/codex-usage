...
浮窗会先读取已有缓存，再同步数据。首次运行可能需要等待 ccusage 下载。点击浮窗外部会关闭浮窗；进行中的同步由后台工作线程持有，再次打开会复用该请求。
...
每个北京时间自然日的首次同步会向 npm 查询 ccusage 最新版本，再运行查询到的版本。通过校验的结果成为新报告，该版本成为当前可用版本。候选版本执行失败或输出不兼容时，会尝试最近成功版本；仍失败则保留旧报告并提示错误。浏览器关闭时不运行定时采集。
...
本地主机使用查询到的 ccusage 版本执行 `daily --by-agent --json --timezone Asia/Shanghai`，采用独立配置并启用在线价格获取，从包含逐模型费用的统一报表中提取 Codex 分组。
...
现有 6 项自动化测试覆盖数据规范化、未知价格、异常输出、版本回退、每日更新检查、缓存保留和 Native Messaging 分帧。当前紧凑界面已用本地缓存数据验证日期／模型筛选、指标／布局切换、明细展开和首屏尺寸。重新设计的浮窗及图标仍需在重新加载后通过实际 Edge 工具栏验证。
...
| 同步失败 | 检查网络及 Node／npm／npx 是否可用；使用上面的命令行同步命令排查。缓存保留原来的同步时间。 |
...
可选删除 `~/.codex-usage-edge/`，清除缓存状态和启动脚本。Codex 日志不会被修改。
