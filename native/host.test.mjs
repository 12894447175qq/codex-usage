...
test('新版校验失败回退、每天检查一次、完全失败保留缓存、协议分帧', async () => {
  ...
  try {
    ...
    await writeFile(join(dir, 'npx'), `#!${process.execPath}\nconst fs=require('fs');if(process.env.TEST_FAIL)process.exit(1);console.log(process.argv.includes('ccusage@2.0.0')?'{}':fs.readFileSync(process.env.TEST_DIR+'/raw.json','utf8'));`, { mode: 0o700 });
